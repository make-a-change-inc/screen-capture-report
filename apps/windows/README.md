...
5. 生画像は本人がローカルアーカイブで確認できるよう暗号化したまま最大24時間保持し、その後削除します。失敗した解析はSQLiteキューに残り、ネットワーク復旧・再起動後に再試行します。
...
オンボーディングで企業コード、従業員識別子、部署を入力すると、初回同期時に端末を管理Webへ自動登録します。端末トークンはCloudflareから発行され、Windowsの秘密ストアへ保存されるため、管理画面での事前登録や`*.scr-provision.json`の受け渡しは不要です。
...
オンボーディングまたは設定画面で企業コードを入力し、管理Web同期へ同意した場合だけ、確定済みの管理者向け週次レポートを送信します。本人日報、キャプチャ画像、ウィンドウタイトルは送信しません。
