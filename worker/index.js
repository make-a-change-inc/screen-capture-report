...
async function requireAdmin(request, env) {
  ...
  return timingSafeMatch(key, env.BOOTSTRAP_ADMIN_HASH || "");
}
...
async function uploadReport(request, env) {
  ...
  if (!report.reportId || !/^\d{4}-\d{2}-\d{2}$/.test(report.periodStart)
...
const page = `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Screen Capture Report</title><style>
...
export { encryptReport, decryptReport, normalizeReport, sha256, timingSafeMatch };
